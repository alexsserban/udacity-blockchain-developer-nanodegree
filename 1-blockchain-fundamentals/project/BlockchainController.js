...
class BlockchainController {
    ...
    isChainValid() {
        this.app.get("/isChainValid", async (req, res) => {
            let chainErrorLog = await this.blockchain.validateChain();
            if (chainErrorLog.length === 0) {
                return res.status(200).json({ result: true, chainErrorLog });
            } else {
                return res.status(500).send({ response: false, chainErrorLog });
            }
        });
    }
}
